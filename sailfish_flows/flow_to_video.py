...
# make video
success = video.open('flow_video.mov', fourcc, 8, ((shape[1]-2)/2, (shape[0]-2)/2), True)
...
for f in files:
  ...
  if len(x.f.v.shape) == 4:
    frame = np.sqrt(np.square(x.f.v[0,:,shape[2]/2,:]) + np.square(x.f.v[1,:,shape[2]/2,:]) + np.square(x.f.v[2,:,shape[2]/2,:]))
