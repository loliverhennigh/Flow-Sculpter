...
Up until now we have not discussed how to generate data to train these neural neural networks. Generateing data to train the parameterization network is realitively simple. If the paraeterization we are using is known we simply neet to make a set of parameter vectors and there corissponding geometrys. In the case of the heat sink this is a set of examples composed of the 15 parameters and there corrisponding binary representation of the head sink. Putting together a dataset for $S^{heat}_{net}$ or $S^{flow}_{net}$ (fluid flow network) is somewhat of a more complicated task though. The simplest solution and the apporoch used in this work is to simply simulate the respective physics on objects drawn from the di
